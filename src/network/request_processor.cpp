...
RequestProcessor::RequestProcessor(PersistentKeyValueStore& store,
...
PutResult RequestProcessor::put(std::string key, std::string value) noexcept {
  try {
    ...
    return {false, store_.last_sequence_number(),
            {ApiErrorCode::Internal, "unknown storage error"}};
  }
}
...
BatchPutResult RequestProcessor::batch_put(
  ...
  for (auto& [key, value] : entries) {
    ...
    if (!result.success) {
      return {false, writes_applied, store_.last_sequence_number(),
...
StatusSnapshot RequestProcessor::status(
  ...
  snapshot.level0_sstables =
      static_cast<std::uint64_t>(store_.level0_sstable_count());
  snapshot.level1_sstables =
      static_cast<std::uint64_t>(store_.level1_sstable_count());
  ...
  snapshot.queued_requests =
      static_cast<std::uint64_t>(executor.queued_tasks);
  snapshot.active_requests =
      static_cast<std::uint64_t>(executor.active_tasks);
...
ApiError RequestProcessor::validate_batch(
  ...
  if (entries.size() > options_.max_batch_entries) {
    return {ApiErrorCode::InvalidArgument,
            "batch exceeds the maximum number of entries"};
  }
  ...
  try {
    for (const auto& [key, value] : entries) {
      ...
      if (key.size() > std::numeric_limits<std::size_t>::max() - total_bytes ||
          value.size() >
              std::numeric_limits<std::size_t>::max() - total_bytes - key.size()) {
      ...
      if (total_bytes > options_.max_batch_bytes) {
        return {ApiErrorCode::InvalidArgument,
                "batch exceeds the maximum encoded payload size"};
      }
    }
  } catch (const std::exception& error) {
