...
[[nodiscard]] std::vector<std::byte> read_exact(const int descriptor,
  ...
  while (completed < size) {
    ...
    const ssize_t result =
        ::pread(descriptor, bytes.data() + completed, size - completed,
...
SSTableReader::SSTableReader(
    ...
    : path_{std::move(path)},
      cache_table_id_{path_.lexically_normal().string()},
  ...
  const std::uint64_t minimum_size =
      sstable_format::kHeaderSize + sstable_format::kFooterSize;
  ...
  const auto header_bytes =
      read_exact(descriptor.get(), 0, sstable_format::kHeaderSize);
  const sstable_format::Header header =
      sstable_format::parse_header(header_bytes, 0);

  const std::uint64_t footer_offset = size - sstable_format::kFooterSize;
  const auto footer_bytes = read_exact(descriptor.get(), footer_offset,
                                       sstable_format::kFooterSize);
  footer_ = sstable_format::parse_footer(footer_bytes, footer_offset);

  if (footer_.index_offset < sstable_format::kHeaderSize ||
      footer_.index_offset > footer_offset ||
      footer_.index_size > footer_offset - footer_.index_offset) {
    throw SSTableCorruptionError{footer_offset,
                                 "SSTable footer references an invalid index range"};
  }
  if (header.entry_count != footer_.entry_count ||
      header.block_count != footer_.block_count ||
      header.min_sequence_number != footer_.min_sequence_number ||
      header.max_sequence_number != footer_.max_sequence_number) {
    throw SSTableCorruptionError{footer_offset,
                                 "SSTable header and footer metadata disagree"};
  }
  ...
  const auto index_bytes =
      read_exact(descriptor.get(), footer_.index_offset,
  ...
  for (const IndexEntry& entry : index_.entries) {
    if (entry.block_offset < sstable_format::kHeaderSize ||
        entry.block_offset < previous_end || entry.block_offset > footer_.index_offset ||
...
std::optional<Entry> SSTableReader::get(const std::string_view key) const {
  ...
  const auto candidate = std::lower_bound(
      index_.entries.begin(), index_.entries.end(), key,
...
bool SSTableReader::uses_bloom_filter() const noexcept {
  return static_cast<bool>(bloom_filter_);
}

std::optional<BloomFilterStatistics> SSTableReader::bloom_filter_statistics()
    const noexcept {
  if (!bloom_filter_) {
    return std::nullopt;
  }
...
SSTableLookupStatistics SSTableReader::lookup_statistics() const noexcept {
  return SSTableLookupStatistics{
      bloom_checks_.load(std::memory_order_relaxed),
...
BlockCache::BlockPointer SSTableReader::read_data_block(
  ...
  if (block->first_key() != index_entry.first_key ||
      block->last_key() != index_entry.last_key) {
    throw SSTableCorruptionError{index_entry.block_offset,
